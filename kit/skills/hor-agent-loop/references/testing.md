...
These follow the **`hoc-jest` skill** — read it for the general rules; this file only
...
the test file is unverified logic (see the `hoc-jest` skill's anti-pattern); if the construction is
