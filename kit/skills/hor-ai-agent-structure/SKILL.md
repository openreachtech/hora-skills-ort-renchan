...
the `@openreachtech/mentsu-agent-loop-core` package family; see the **`hor-agent-loop` skill** for those. Do
...
- **Why load the tool schema from the DB**: the tool JSON (name, description, parameters) lives in the
  ...
  a data change, not a deploy. See the **`hor-ai-prompt-document-store` skill**.
...
| `processor` | the LLM processor (see the `hor-multi-llm-provider` skill) |
...
**`hor-agent-loop` skill** (`renchan-job` / `graphql` adapters) instead of the synchronous runner.
