...
declaration (migration files, DataTypes, indexes) is written by `hor-sequelize-migration`; the logical
declaration on top (model attributes, associations) by `hor-sequelize-model`. This skill decides
...
This skill is design-level. Whenever a decision needs to be *written*, cross to `hor-sequelize-migration`
(column types, `DATE(3)`, `TEXT('medium')`, `JSON`, no DB foreign-key constraint, indexes) and
`hor-sequelize-model` (attributes, timestamps, and the Mixins that implement versioning and archiving).
