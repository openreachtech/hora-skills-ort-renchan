...
[SKILL.md](../SKILL.md). The mixin that implements this is written per `hor-sequelize-model`.
...
- **Mechanics**: renchan implements the archive pattern with `BackupMixinModel` + a `*_bk` table
  ...
  Pass the mixin from the live model; see the mixin catalog in `hor-sequelize-model`.
