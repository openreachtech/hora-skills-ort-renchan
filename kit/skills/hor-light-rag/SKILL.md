...
> `hor-ai-agent-structure` skill's execute step. Because *document* search is not on Elasticsearch and there is no
...
part 1 of the composed instruction — see the **`hor-ai-prompt-document-store` skill**.
...
**`hor-ai-prompt-document-store` skill** for the full model set, migrations, and seeders, and the
