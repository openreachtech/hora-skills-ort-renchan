...
The physical schema is defined by `hor-sequelize-migration`; this
...
- **Do not "delete to omit"**. Removing a method entirely makes it impossible to tell whether
  ...
  the QA stance in `hoc-jest` ("do not cut coverage using implementation
...
`hor-sequelize-migration`. The camelCase attribute keys on the model side map
