...
Two codes make the cookie flow legible to the frontend ([[hof-cookie-authentication]]), on top of the engine's standard hash. Codes follow the dotted `<class>.<id>.<seq>` convention, spread over `super.errorCodeHash` in the resolver.
