...
entity's key column is written per `hor-sequelize-migration`; the versioning mixin per `hor-sequelize-model`.
...
- The entity's key column is an **FK-like column** (uppercase-initial `OrderStatusId`, **no DB
  foreign-key constraint**); follow the FK-like-column rule in `hor-sequelize-migration`.
...
- **Why `is_active` instead of deleting a row**: retiring a value by flag keeps the row — and
  ...
  constraint to stop it; integrity is the app's job, per `hor-sequelize-migration`). Filter to
...
- **Mechanics**: renchan implements exactly this with `SuiteVersionMixinModel` — the version table
  ...
  effect, and `findCurrentSuite()` reads it. See the mixin catalog in `hor-sequelize-model`.
